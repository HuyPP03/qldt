...
export default function EditFile() {
  ...
  const { id, classId } = route.params as { id: string; classId: string };
  ...
  const getMaterial = async () => {
    try {
      const response: any = await request(
        `${SERVER_URL}/it5023e/get_material_info`,
        {
          method: "POST",
          body: {
            token,
  ...
  const handleUploadFile = async () => {
    ...
    try {
      ...
      const response = await request(`${SERVER_URL}/it5023e/edit_material`, {
        method: "POST",
        headers: {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerText}>Edit file</Text>
      </View>
      {loading ? (
